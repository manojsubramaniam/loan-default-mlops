...
input_data = {
    ...
    "Accompany_Client": st.selectbox("Accompany Client", ["Alone", "Family", "Partner"]),
    "Client_Income_Type": st.selectbox("Income Type", ["Working", "Commercial associate", "Pensioner"]),
    "Client_Education": st.selectbox("Education", ["Higher education", "Secondary", "Incomplete higher"]),
    "Client_Marital_Status": st.selectbox("Marital Status", ["Single", "Married", "Divorced"]),
    "Client_Gender": st.selectbox("Gender", ["M", "F"]),
    "Loan_Contract_Type": st.selectbox("Loan Contract Type", ["Cash loans", "Revolving loans"]),
    "Client_Housing_Type": st.selectbox("Housing Type", ["House", "With parents", "Rented apartment"]),
    "Population_Region_Relative": st.number_input("Population Region Relative", value=0.01),
    ...
    "Client_Occupation": st.selectbox("Occupation", ["Laborers", "Core staff", "Managers"]),
    "Client_Permanent_Match_Tag": st.selectbox("Permanent Match Tag", ["Y", "N"]),
    "Client_Contact_Work_Tag": st.selectbox("Contact Work Tag", ["Y", "N"]),
    "Type_Organization": st.selectbox("Type of Organization", ["Business Entity Type 1", "Business Entity Type 2"]),
...
if st.button("Predict"):
    ...
    try:
        response = requests.post("http://api:8000/predict", json=input_data, headers=headers)
        if response.status_code == 200:
            result = response.json()
            st.success(f"Prediction: {result['prediction']}")
            st.info(f"Probability of Default: {result['probability_of_default'] * 100:.2f}%")
        else:
